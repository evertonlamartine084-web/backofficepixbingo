  ...
  try {
    ...
    if (action === 'data') {
      ...
      const [achievements, missions, tournaments, wheelPrizes] = await Promise.all([
        applySegmentFilter(achievementsQ),
        applySegmentFilter(missionsQ),
        applySegmentFilter(tournamentsQ),
        applySegmentFilter(wheelPrizesQ),
      ]);

      // Mini games
      let miniGamesQ = supabase.from('mini_games').select('id, type, name, description, theme, config, max_attempts_per_day, free_attempts_per_day, attempt_cost_coins, segment_id, segments(name)')
        .eq('active', true);
      const miniGamesResult = await applySegmentFilter(miniGamesQ);
      const miniGames = miniGamesResult.data || [];

      // Get prizes for all mini games
      ...
      // Also get levels, store items, wheel config, and player data
      ...
      if (playerCpf) {
        extraQueries.push(
          supabase.from('player_wallets').select('*').eq('cpf', playerCpf).maybeSingle(),
          supabase.from('player_spins').select('*').eq('cpf', playerCpf).maybeSingle(),
          supabase.from('player_mission_progress').select('*').eq('cpf', playerCpf),
          supabase.from('player_achievements').select('*').eq('cpf', playerCpf),
          supabase.from('player_activity_log').select('*').eq('cpf', playerCpf).order('created_at', { ascending: false }).limit(50),
          supabase.from('player_rewards_pending').select('*').eq('cpf', playerCpf).is('claimed_at', null),
          supabase.from('player_tournament_entries').select('*').eq('cpf', playerCpf),
          supabase.from('player_mini_game_attempts').select('*').eq('cpf', playerCpf),
        );
      }
      const results = await Promise.all(extraQueries);
      const [levels, storeItems, wheelConfigResult] = results;
      ...
      if (playerCpf) {
        ...
        const spinsResult = results[4];
        const missionProgressResult = results[5];
        const achievementProgressResult = results[6];
        const activityLogResult = results[7];
        const pendingRewardsResult = results[8];
        const tournamentEntriesResult = results[9];
        const miniGameAttemptsResult = results[10];
    ...
    if (action === 'store_buy') {
      ...
      if (item.price_coins > 0) {
        await supabase.from('player_wallets')
          .update({ coins: coins - item.price_coins } as any)
          .eq('cpf', playerCpf);
      }
      ...
      return new Response(JSON.stringify({ success: true, item_name: item.name }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
